...
import 'interfaces/i_read_history_service.dart';
...
class ReadHistoryService extends BaseService implements IReadHistoryService {
  ...
  Future<ShelfItemDto> setReadHistory(ReadHistoryDto readHistory) async {
    ShelfItemDto? shelfItemDto =
        await _shelfItemService.getShelfItemById(readHistory.bookId);
    ...
    if (shelfItemDto.readingStatus != ReadingStatus.reading) {
      ...
      _shelfItemService.updateReadStatus(
          shelfItemDto.bookId, ReadingStatus.reading);
    }
    ...
    final currentHistoryDto = shelfItemDto.readHistory
        .firstWhereOrNull((h) => h.id == readHistory.id);

    if (currentHistoryDto != null) {
      await _readHistoryRepository.updateReadHistory(
          currentHistoryDto.id!,
          shelfItemDto.bookId,
          userId,
          readHistory.pages!,
          readHistory.percentage!);
    } else {
    ...
    if (readHistory.percentage == 100) {
      shelfItemDto = await _shelfItemService.updateReadStatus(
          shelfItemDto.bookId, ReadingStatus.read);
    }
  ...
  @override
  Future<ShelfItemDto> removeReadHistory(
      String bookId, String historyId) async {
    final userId = await getUserId();
