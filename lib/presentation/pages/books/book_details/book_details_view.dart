...
import '../../../blocs/books/book_detail/book_detail_bloc.dart';
...
class BookDetailWidget extends StatelessWidget {
        ...
        Padding(
          ...
          child: ClipRRect(
            ...
            child: ImageWithShimmer(
              ...
              imageUrl: bookDetail.imageLinks.thumbnail ?? 'default_image_url',
            ),
          ),
        ),
        Positioned(
    ...
    return Padding(
      ...
      child: Row(
        ...
        children: [
          Flexible(
            flex: 7,
            child: Text('${bookDetail.publisher} ${bookDetail.publisherYear()}', style: textTheme.bodyLarge),
          ),
          const CircleDot(),
          Flexible(
            flex: 3,
            child: Text('${bookDetail.totalPages} páginas', style: textTheme.bodyLarge),
          ),
        ],
      ),
    );
  ...
  Widget _buildSynopsis(BuildContext context, TextTheme textTheme, Size size, BookDetail bookDetail) {
    return Padding(
              ...
              children: [
                ...
                if (bookDetail.readingStatus != null) ...[
                  Container(
                    ...
                    child: GestureDetector(
                      onTap: () {
                        context.pushNamed(AppRoutes.bookOptionsRoute, pathParameters: {'bookId': bookDetail.id});
                      },
                      child: const Icon(
                        Icons.edit,
  ...
  Widget _buildBackButton(BuildContext context) {
    return Padding(
      padding: const EdgeInsets.only(top: AppPadding.p12, left: AppPadding.p16, right: AppPadding.p16),
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
          InkWell(
  ...
  void _showModal({required BuildContext context, required BookDetail item}) {
    final shelfItem = ShelfItem(
      bookId: item.id,
      imageUrl: item.imageLinks.thumbnail ?? 'default_image_url',
  ...
  Widget _buildBottomSheet(BuildContext context, BookDetailBloc bloc, ShelfItem item) {
    return Container(
      padding: const EdgeInsets.all(16.0),
      child: Column(
        mainAxisSize: MainAxisSize.min,
        children: [
          _buildListTile(context, bloc, item, AppStrings.read, Icons.bookmark, AppColors.read, ReadingStatus.read),
          _buildListTile(context, bloc, item, AppStrings.reading, Icons.bookmark, AppColors.reading, ReadingStatus.reading),
          _buildListTile(context, bloc, item, AppStrings.wantToRead, Icons.bookmark, AppColors.wantToRead, ReadingStatus.wantToRead),
          _buildListTile(context, bloc, item, AppStrings.rereading, Icons.bookmark, AppColors.rereading, ReadingStatus.rereading),
          _buildListTile(context, bloc, item, AppStrings.abandoned, Icons.bookmark, AppColors.abandoned, ReadingStatus.abandoned),
        ],
      ),
    );
  }
